...
const App: React.FC = () => {
  ...
  const handleInputChange = (id: number, field: keyof Omit<ScriptRowData, 'id' | 'voice' | 'audio' | 'duration' | 'isLoading' | 'error' | 'pause'>, value: string) => {
  ...
  const handleGenerateSpeech = useCallback(async (row: ScriptRowData) => {
    ...
    const { id, style, script, voice } = row;

    updateRow(id, { isLoading: true, error: null, audio: null, duration: null });

    try {
      const fullPrompt = style ? `Hãy đọc với giọng ${style}: ${script}` : script;
  ...
  const handlePlayAll = async () => {
    ...
    try {
      ...
      const playNextInQueue = () => {
        ...
        source.onended = () => {
          // Defensive check: This ensures that the onended handler for a stale or
          // manually stopped source doesn't incorrectly queue the next track.
          // stopAllPlayback() nullifies playAllSourceRef.current, which stops the chain.
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <main>
          <div className="bg-gray-800 rounded-lg shadow-lg">
            ...
            <div className="grid grid-cols-[50px_1.5fr_3fr_1.5fr_120px_320px_auto] gap-4 items-center p-4 font-semibold text-sm text-gray-400 bg-gray-900/50">
            ...
            <div className="divide-y divide-gray-700">
             {scriptRows.map((row, index) => (
                <div key={row.id} className={`grid grid-cols-[50px_1.5fr_3fr_1.5fr_120px_320px_auto] gap-4 items-center p-4 hover:bg-gray-700/50 transition-colors ${
